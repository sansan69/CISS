...
exports.processEmployeeCSV = functions
    .runWith(runtimeOpts)
    ...
    cors(req, res, async () => {
        ...
        busboy.on("finish", async () => {
            ...
            for (const emp of employeesToProcess) {
                try {
                    ...
                    if (emp.joining_date && !isNaN(new Date(emp.joining_date).getTime())) {
                        employeeData.joiningDate = firestore_1.Timestamp.fromDate(new Date(emp.joining_date));
                    }
                    else {
                        console.warn(`Invalid or missing JoiningDate for ${employeeData.fullName}, using current date as fallback.`);
                        employeeData.joiningDate = firestore_1.Timestamp.now(); // Fallback or handle as error
                    }
                    if (emp.date_of_birth && !isNaN(new Date(emp.date_of_birth).getTime())) {
                        employeeData.dateOfBirth = firestore_1.Timestamp.fromDate(new Date(emp.date_of_birth));
                    }
                    else {
                    ...
                    const safeIdentifier = employeeData.phoneNumber || (0, uuid_1.v4)();
                    ...
                    if (emp.BankPassbookPhotoBlob &&
                        ...
                        if (matches && matches.length === 3) {
                            ...
                            console.warn(`Invalid BankPassbookPhotoBlob format for ${employeeData.fullName}, using BankPassbookStatementURL if present.`);
                        }
                    }
                    else {
                ...
                catch (processingError) { // Changed from 'any' to 'unknown' for better practice
            ...
            for (let i = 0; i < processedEmployeesForFirestore.length; i += batchSize) {
                ...
                catch (dbError) { // Changed from 'any' to 'unknown'
...
//# sourceMappingURL=processEmployeeCSV.js.map
