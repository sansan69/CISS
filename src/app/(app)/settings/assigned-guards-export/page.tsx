...
export default function AssignedGuardsExportPage() {
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            // Avoid orderBy to minimize index needs; we'll sort locally
            const woSnap = await getDocs(wq);
            let workOrders: WorkOrderDoc[] = woSnap.docs.map(d => ({ id: d.id, ...(d.data() as any) })) as any;

            // If filtering by field officer, keep orders in officer's assigned districts
            ...
            // Fetch employees referenced in assignedGuards
            ...
            // Batch fetch in chunks of 10 for 'in' queries
            ...
            const headers = [
                'Sl No.', 'State', 'City', 'Center Name', 'Center code', 'First Name of the employee', 'Last Name of the employee', 'male/female', 'date of birth', "father name", "mother name", 'Full address', 'contact number', 'email id', 'Resources ID (If available)', 'ID Proof Type', 'ID proof number'
            ];

            const wb = XLSX.utils.book_new();
            const ws = XLSX.utils.aoa_to_sheet([headers, ...rows]);
            // Format DoB column as date
